...
Queue.prototype.processJobs = function(concurrency){
  ...
  process.nextTick(function() {
    if(!_this.closing){
      (_this.paused || Promise.resolve()).then(function(){
        console.time('about to call moveToActive --> pre-Job.fromData')
        ...
        return scripts.moveToActive(_this).spread(function(jobData, jobId){
          if(jobData){
            console.timeEnd('about to call moveToActive --> pre-Job.fromData')
            console.timeEnd('new queue created --> pre-Job.fromData');
