...
Queue.prototype.setHandler = function(concurrency, handler){
  ...
  if(handler.length > 1){
    this.handler = Bluebird.promisify(handler);
  }else{
    this.handler = Bluebird.method(handler);
  }
};
...
Queue.prototype.processStalledJobs = function(){
  ...
  return this.client.lrangeAsync(this.toKey('active'), 0, -1).then(function(jobs){
    return Bluebird.each(jobs, function(jobId) {
...
Queue.prototype.processJob = function(job){
  ...
  var jobPromise = Bluebird.resolve(this.handler(job));
...
Queue.prototype.clean = function (grace, type) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this[getter]().then(function (jobs) {
      ...
      return Bluebird.filter(jobs, function (job) {
        return job && ((!job.timestamp) || (job.timestamp < Date.now() - grace));
      });
    }).then(function (jobs) {
      //remove those old jobs
      return Bluebird.each(jobs, function (job) {
        return job.remove();
      });
    }).then(function (jobs) {
