...
var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  ...
  getRedisVersion(this.client).then(function getRedisVersion(version){
  ...
  }).catch(function failedGetRedisVersion(err){
  ...
  this.client.selectAsync(redisDB).then(function bclientSelectAsync(){
    return _this.bclient.selectAsync(redisDB);
  }).then(function eclientSelectAsync(){
    return _this.eclient.selectAsync(redisDB);
  }).then(function eclientSubscribeAsync(){
    return _this.eclient.subscribeAsync(_this.toKey('delayed'));
  }).then(function eclientSubscribeAsyncPaused(){
  ...
  updateDelaySet(this, Date.now()).then(function updateDelaySet(timestamp){
    if(timestamp){
  ...
  this.guardianTimer = setInterval(function() {
    if(_this.delayedTimestamp < Date.now() || _this.delayedTimestamp - Date.now() > POLLING_INTERVAL){
      updateDelaySet(_this, Date.now()).then(function updateDelaySet(timestamp){
        if(timestamp){
          _this.updateDelayTimer(timestamp);
        }
      }).catch(function failedUpdateDelaySet(err){
        console.error(err);
      });
    }
  }, POLLING_INTERVAL);
...
Queue.prototype.process = function(concurrency, handler){
  ...
  return this.run().catch(function processFailed(err){
    console.log(err);
    throw err;
  });
};
...
Queue.prototype.add = function(data, opts){
  ...
  //
  return _this.client.incrAsync(this.toKey('id')).then(function incrAsyncJob(jobId){
    return Job.create(_this, jobId, data, opts);
  }).then(function jobCreated(job){
    if(job.delay){
      var jobDelayedTimestamp = job.timestamp + job.delay;
      if(jobDelayedTimestamp > Date.now()){
        return job._addToDelayed(jobDelayedTimestamp).then(function addedToDelayed(){
          _this.emit('delayed', job);
          return job;
        });
      }
    }
    return addJob(_this, job.jobId, opts).then(function emitWarning(){
...
Queue.prototype.count = function(){
  ...
  return multi.execAsync().then(function execAsync(res){
...
Queue.prototype.updateDelayTimer = function(newDelayedTimestamp){
  ...
  if(newDelayedTimestamp < _this.delayedTimestamp){
    ...
    this.delayTimer = setTimeout(function(){
      updateDelaySet(_this, _this.delayedTimestamp).then(function updateDelaySet(nextTimestamp){
        if(nextTimestamp){
...
Queue.prototype.processStalledJobs = function(){
  var _this = this;
  return this.client.lrangeAsync(this.toKey('active'), 0, -1).then(function lrangeAsync(jobs){
...
Queue.prototype.processStalledJob = function(job){
  ...
  if(!job){
    ...
    return job.takeLock(_this.token).then(function takeLock(lock){
      if(lock){
        var key = _this.toKey('completed');
        return _this.client.sismemberAsync(key, job.jobId).then(function sismemberAsync(isMember){
          if(!isMember){
...
Queue.prototype.takeLockAndProcessJob = function(job){
  ...
  var _this = this;
  return job.takeLock(this.token).then(function takeLock(locked) {
    if(locked){
      return _this.processJob(job);
    }
  });
...
Queue.prototype.processJob = function(job){
  ...
  var lockRenewer = function(){
    job.renewLock(_this.token).catch(function failedRenewLock(err){
      console.error(err);
    });
  ...
  function handleCompleted(data){
    ...
    _this.processing--;
    return job.moveToCompleted(data).then(function moveToCompleted(){
  ...
  return jobPromise
    ...
    .then(function jobPromise(){
      _this.timers.clear(lockRenewId);
    }).catch(function failedJobPromise(){
      _this.timers.clear(lockRenewId);
    });
};
...
Queue.prototype.moveJob = function(src, dst, opts) {
  if(opts && opts.block === false){
    ...
    return this.bclient.brpoplpushAsync(this.toKey(src), this.toKey(dst),
                Math.floor(this.LOCK_RENEW_TIME / 1000)).then(function brpoplpush(jobId) {
...
Queue.prototype.getJobCountByTypes = function() {
  ...
  return multi.execAsync().then(function multiExecAsync(res){
...
Queue.prototype.getJobs = function(queueType, type, start, end){
  ...
  return jobs.then(function allJobs(jobIds){
...
Queue.prototype.clean = function (grace, type) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this[getter]().then(function thisGetter(jobs) {
...
var getRedisVersion = function getRedisVersion(client){
  return client.infoAsync().then(function infoAsync(doc){
