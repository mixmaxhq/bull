...
Queue.prototype.processJobs = function(concurrency){
  ...
  process.nextTick(function() {
    if(!_this.closing){
      (_this.paused || Promise.resolve()).then(function(){
        console.time('about to call moveToActive --> post-Job.fromData')
        console.timeEnd('new queue created --> about to call moveToActive')
        return scripts.moveToActive(_this).spread(function(jobData, jobId){
          if(jobData){
            var job = Job.fromData(_this, jobData, jobId);
            console.timeEnd('about to call moveToActive --> post-Job.fromData')
            console.timeEnd('new queue created --> post-Job.fromData');
            return _this.processJob(job).then(function() {
              return _this.processJobs(concurrency, resolve);
            });
          }
        }).catch(function(err){
          _this.emit('error', err, 'Error processing job');
