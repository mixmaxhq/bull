...
Queue.prototype.run = function(concurrency){
  ...
  while (concurrency--) {
    promises.push(new Promise(this.processJobs));
  }
...
Queue.prototype.getFailed = function(start, end){
  return this.getJobs('failed', 'SET', false, start, end);
};
...
Queue.prototype.getJobs = function(queueType, type, asc, start, end){
  ...
  switch(type){
    case 'LIST':
      if(asc){
        jobs = this.client.lrangeAsync(key, -(end + 1), -(start + 1)).then(function(result){
      ...
      break;
    case 'ZSET':
      jobs = asc ? this.client.zrangeAsync(key, start, end) : this.client.zrevrangeAsync(key, start, end);
      break;
    case 'SET':
  ...
  return jobs.then(function(jobIds){
    ...
    return Promise.all(jobsFromId.map(function(promise) {
      return promise.reflect();
    })).then(function(values) {
      return values.filter(function(inspection) {
